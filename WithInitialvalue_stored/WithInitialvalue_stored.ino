...
void loop(void) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    // Complementary filter - combine acceleromter and gyro angle values
    roll = gyroAngleX + accAngleX; //Calculating  roll 
  
    pitch = gyroAngleY + accAngleY; // Calculating pitch
    ...
    if (pitch < 12) {
      ...
      tof = true;
      if (((initialangle_z - angle_z) < 12)) { // Making the comparison of angle with z-axis 
        tof = true;
      } else if (((initialangle_x - angle_x) < -45) || ((initialangle_x - angle_x) < -110)) { // Making the comparison of angle with x-axis 
        tof = false;
      } else {
        tof = false;
      }
    ...
    if ((gcc_x > 2.5) || (gcc_x < -2.5)) {
      fall = false;
    }
    if ((gcc_y > 2.5 ) || (gcc_y < -2.5)) { //Measuring that user rotate or not 
      fall = false;
    }
    if ((gcc_z > 2.5 ) || (gcc_z < -2.5)) { //Measuring fall of the front and back 
      fall = false;
    }
    if (tof == true) {
      count++;
